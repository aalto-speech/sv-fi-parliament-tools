...
### ! These are custom flags required by recipes 03, 04, and 05
stop_after_preprocessing=false
stop_after_decoding=false
...
if [ ! -z "$extractor" ]; then
  ...
  if [ $stage -le 4 ]; then
    ...
    steps/online/nnet2/extract_ivectors_online.sh \
      --nj $nj_reco --cmd "$cmd --mem 4G" --use-vad $use_vad \
      $data_uniform_seg $extractor $online_ivector_dir
  fi
fi

if $stop_after_preprocessing; then
  echo "Set to stop after preprocessing, exiting."
  exit
fi
...
if [ $stage -le 5 ]; then
  ...
  steps/cleanup/decode_segmentation_nnet3.sh \
    ...
    --beam $beam --lattice-beam $lattice_beam --nj $nj --cmd "$cmd" \
...
if $stop_after_decoding; then
  echo "Set to stop after decoding, exiting."
  exit
fi
...
if [ $stage -le 6 ]; then
  steps/get_ctm_fast.sh --lmwt $lmwt --cmd "$cmd" \
...
if [ $stage -le 10 ]; then
  ...
  $ctm_cmd JOB=1:$nj $decode_dir/ctm_$lmwt/log/get_ctm_edits.JOB.log \
